...
- MFA alignment uses `run.mfa.bin` from the SAPC YAML and the `english_mfa` acoustic model; `mfa version` currently reports `3.3.9` from the `FastSpeech_tts` conda environment, and the PBS script preflights MFA before preparing alignment data.
