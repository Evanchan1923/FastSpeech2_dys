...
- MFA alignment uses `run.mfa.bin` from the SAPC YAML, the `english_mfa` acoustic model, and the `english_us_mfa` dictionary model; `mfa version` has reported `3.3.8`/`3.3.9` from the `FastSpeech_tts` conda environment, and the PBS script preflights MFA plus Kalpy graph-compiler compatibility before preparing alignment data.
