...
- MFA alignment uses `run.mfa.bin` from the SAPC YAML and the `english_mfa` acoustic model; `mfa` plus `kalpy-kaldi` are expected in the FastSpeech2 venv PATH and the PBS script preflights MFA before preparing alignment data.
